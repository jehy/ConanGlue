...
void CConanGlueDlg::OnBnClickedButton4()
{
  ...
  {
    MessageBox("Не удалось прочитать файл спецификации склейки!","Ошибка",MB_OK);
    return;
  }
  CString info;
  CString fname;
  int nRec=FileRecords.size();
  info.Format("%d элементов для обработки",nRec);
  AddLog(&Log,info);
  //begin reading conan
	ProgressBar.SetRange(0, 3+nRec);
  ProgressBar.StepIt();
  
  AddLog(&Log,"создаём файл заголовка");
 ...
 for (int i=0;i<Conan->Header->nRec;i++)
  ...
  for(int CurrentRecNum=0;CurrentRecNum<nRec;CurrentRecNum++)
  {
    ...
    if(NewConan==NULL)
    {
      ...
      info.Append(fname);
      info.Append(" Склейка не может быть завершена");
      MessageBox(info);
      return;
    }
  ...
  {
    MessageBox("Не удалось записать склеенный файл!");
  }
  //clear
	delete(Conan);
  ProgressBar.StepIt();
  AddLog(&Log,"запись окончена");
}
